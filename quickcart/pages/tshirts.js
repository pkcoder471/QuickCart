import React from 'react'
import Link from 'next/link'
const tshirts = () => {
  return (
    <section className="text-gray-600 body-font">
      <div className="container px-16 py-16 mx-auto">
        <div className="flex flex-wrap -m-4">
          
            <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block" src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
          <Link href={'/product/hoodie'} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-sm cursor-pointer "><div>
            <a className="  relative h-80 m-auto rounded overflow-hidden">
              <img alt="ecommerce"  className="h-80 m-auto block  " src="https://m.media-amazon.com/images/I/61b4R3TMXoL._SY879_.jpg"/>
            </a>
            <div className="mt-4">
              <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">CATEGORY</h3>
              <h2 className="text-gray-900 title-font text-lg font-medium">Tshirts</h2>
              <p className="mt-1">₹499</p>
            </div>
          </div></Link>
        </div>
      </div>
    </section>
  )
}

export default tshirts
